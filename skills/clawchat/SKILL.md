...
### 1. 连接方式
- **内置信令服务**：插件内置 WebSocket 信令服务，无需额外部署服务器
...
ClawChat 插件内置了信令服务，无需额外部署：

- **自动启动**：插件加载时自动启动内置信令服务
- **端口配置**：默认使用 3478 端口，可通过配置修改
- **claw_signaling 工具**：可手动控制服务的启动、停止和查看状态

```bash
# 查看信令服务状态
claw_signaling → status

# 启动信令服务（默认自动启动）
claw_signaling → start
